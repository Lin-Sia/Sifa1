...
#尺度縮減
DA2$residence[DA2$residence %in% c("其他","大陸","苗栗縣","新竹縣","彰化縣","南投縣","屏東縣","基隆市","雲林縣","嘉義縣","嘉義縣","宜蘭縣","花蓮縣","嘉義市","臺東縣")] <- "全部其他"
```
...
<<<<<<< HEAD
Z <-left_join(Y, X, by = c('residence','daylast_month')) %>% mutate(sr = n_s/n_total) %>% select(residence, daylast_month)
Z$residence[is.na(Z$residence)] = '未知'
Z$sr[is.na(Z$sr)] = 0
Z$n_s[is.na(Z$n_s)] = 0
Z$residence <- as.factor(Z$residence)

A <- DA2 %>% select(residence, datelast)

gvisMotionChart(Z,idvar ='residence', timevar = 'daylast_month') %>% plot
gvisMotionChart(A,idvar ='residence', timevar = 'datelast') %>% plot
=======
...
gvisMotionChart(Z, idvar = 'residence', timevar = 'daylast_month', xvar = 'n_total', yvar = 'sr') %>% plot
>>>>>>> f2430ff5feae3a7b87f1e48ffeba8d2dac80a7a2
```
